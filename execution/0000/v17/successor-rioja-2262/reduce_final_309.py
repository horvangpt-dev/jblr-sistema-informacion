...
if sha(SRC)!=EXPECTED_SHA:
    violations.append({'type':'SOURCE_SHA_MISMATCH','actual':sha(SRC),'expected':EXPECTED_SHA})
if len(rows)!=309:
    violations.append({'type':'ROW_COUNT','actual':len(rows),'expected':309})
if positions!=list(range(1,310)):
    violations.append({'type':'POSITION_SET_MISMATCH'})
if len(set(source_ids))!=309:
    violations.append({'type':'SOURCE_ID_DUPLICATION','unique':len(set(source_ids))})
if counts!={'RESOLVED':47,'CONFLICT':48,'UNRESOLVED':214}:
    violations.append({'type':'CATEGORY_COUNTS','actual':counts})
...
min_path=OUTDIR/'ALL_309_IDENTITY_MIN.csv'
...
with min_path.open('w',encoding='utf-8',newline='') as f:
    w=csv.writer(f)
    w.writerow(['position309','B_SOURCE_RECORD_ID','nameVerbatim','sourceRank','isHybrid','finalCategory','ID_TAXON_EXACT','state'])
    for r in rows:
        w.writerow([r['position309'],r['B_SOURCE_RECORD_ID'],r['nameVerbatim'],r['sourceRank'],r['isHybrid'],r['finalCategory'],r['ID_TAXON_EXACT'] or '',r['state'] or ''])

integration_keys={
    'resolved':[
        {
            'sourceId':r['B_SOURCE_RECORD_ID'],
            'name':r['nameVerbatim'],
            'rank':r['sourceRank'],
            'idTaxon':str(r['ID_TAXON_EXACT']),
            'state':r['state']
        }
        for r in rows if r['finalCategory']=='RESOLVED'
    ],
    'conflictSourceIds':[r['B_SOURCE_RECORD_ID'] for r in rows if r['finalCategory']=='CONFLICT'],
    'unresolvedSourceIds':[r['B_SOURCE_RECORD_ID'] for r in rows if r['finalCategory']=='UNRESOLVED']
}
(OUTDIR/'INTEGRATION_KEYS_309.json').write_text(json.dumps(integration_keys,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
(OUTDIR/'RESOLVED_47_DIAGNOSTICS.json').write_text(json.dumps(resolved_diagnostics,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
summary={
    'scope':len(rows),
    'counts':counts,
    'uniqueSourceIds':len(set(source_ids)),
    'sourceSha256Actual':sha(SRC),
    'sourceSha256Expected':EXPECTED_SHA,
    'all309Sha256':sha(all_path),
    'resolved47Sha256':sha(res_path),
    'all309IdentityMinSha256':sha(min_path),
    'integrationKeysSha256':sha(OUTDIR/'INTEGRATION_KEYS_309.json'),
