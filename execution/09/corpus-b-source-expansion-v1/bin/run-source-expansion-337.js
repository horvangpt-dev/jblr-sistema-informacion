...
async function main(){
  ...
  const runId=process.env.RUN_ID||'09_CORPUS_B_SOURCE_EXPANSION_337_20260824_001';
  ...
  liveTransports.MITECO_EIDOS=async(name)=>{
    ...
    let r;
    try{r=await eidosBase(name);}catch(e){r={state:'ACCESS_FAILED',accessDetail:String(e.message||e),evidencePointer:inventory.MITECO_EIDOS?.endpoint||null};}
  ...
  liveTransports.HVMO_UIB=async(name)=>{
    ...
    let page=hvmoPageCache.get(key);
    if(!page){
      try{const resp=await timeoutFetch(url,{headers:{accept:'text/html','user-agent':'JBLR-09-SOURCE-EXPANSION/1.0'}}); if(!resp.ok) return {state:'ACCESS_FAILED',accessDetail:`HTTP_${resp.status}`,evidencePointer:url}; page=transportsLib.htmlText(await resp.text()); hvmoPageCache.set(key,page);}catch(e){return {state:'ACCESS_FAILED',accessDetail:String(e.message||e),evidencePointer:url};}
    }
  ...
  const artifacts={
    ...
    'CORPUS_B_337_SOURCE_ACCESS_FAILURE_LEDGER_v1.json':{RUN_ID:runId,rows:sourceAccess},
  ...
  const manifest={RUN_ID:runId,MODE:'CORPUS_B_SOURCE_EXPANSION_337_PRODUCTIVE_09',STATE:'EXECUTED_PENDING_0000',RELEASE_EVENT:'JBLR-EVT-0000-20260824-ACCEPT-08-RELEASE-09-SOURCE-EXPANSION-337-001',DIRECTIVE_ID:'1ZNGdTSTBGLr26_jx4CZiWfBfsOshIKL8_uo9TbkclQ4',INPUT_CURRENT_COLD_START_COMMIT:'fe0dd36a952a24f737495ee7ebe3d57be9e21ee9',IMPLEMENTATION_08_COMMIT:'b04c32c08d7f1fcf47b558efab3ce2a13a4ccf99',IMPLEMENTATION_SOURCE_EXPANSION_SHA:'a751e7449bc817734b0ae67ada0b2a68f99df31e',IMPLEMENTATION_TRANSPORTS_SHA:'678aa7710bcc0b4a32510bbc0f7d7950db90adb2',SOURCE_INVENTORY_SHA:'030ae0e2182375573f49c601a6c64ae38dd8ec62',BRANCH:process.env.GITHUB_REF_NAME||'09-corpus-b-source-expansion-v1',GITHUB_RUN_ID:process.env.GITHUB_RUN_ID||null,GITHUB_RUN_ATTEMPT:process.env.GITHUB_RUN_ATTEMPT||null,STARTED_AT:startedAt,FINISHED_AT:new Date().toISOString(),INPUT_COUNT:337,OUTPUT_COUNT:results.length,COUNTS:counts,QUERY_LEDGER_ROWS:context.queryLedger.length,MITECO_REQUERY_ROWS:mitecoRequery.length,SOURCE_ACCESS_ROWS:sourceAccess.length,SOURCE_PLAN_COMPLETE_ROWS:results.filter(r=>r.SOURCE_PLAN_COMPLETE).length,ARTIFACT_HASHES:hashes,INDEPENDENCE:independence,CORPUS_B_FREEZE:false,CROSS_WITH_A_PERFORMED:false,CROSSWALK_MODULE_EXECUTED:false,NEON_WRITES:0,CANONICAL_CORPUS_A_MUTATION:0};
  writeJson(path.join(outDir,'RUN_MANIFEST.json'),manifest);
  const receipt={receiptVersion:'CORPUS_B_09_SOURCE_EXPANSION_337_RECEIPT_v1',status:'PASS',runId,observedAt:new Date().toISOString(),counts,sourcePlanCompleteRows:manifest.SOURCE_PLAN_COMPLETE_ROWS,independence,hashes:{...hashes,RUN_MANIFEST:shaFile(path.join(outDir,'RUN_MANIFEST.json'))},canonicalEffect:'NONE_PENDING_0000_POSTRUN_ACCEPTANCE'};
  writeJson(path.join(outDir,'RUN_RECEIPT.json'),receipt);
  console.log(JSON.stringify({status:'DONE',runId,counts,queries:context.queryLedger.length,sourcePlanCompleteRows:manifest.SOURCE_PLAN_COMPLETE_ROWS},null,2));
}
