...
function normalizeAccession(row, source, taxon) {
  const originalCode = text(row.original_accession_code || row.catalogNumber || row.catalog_number || row.cod_acces || row.id_acces);
  const sourceId = text(row.source_record_id || row.occurrenceID || row.occurrenceId || row.gbifID || originalCode);
  ...
  return {
    ...
    origin_institution: text(row.origin_institution || row.institutionCode || source.origin_institution || source.publisher) || null,
    original_accession_code: originalCode || null,
    independence_documented: Boolean(originalCode || sourceId),
    ...
    coordinates: row.decimalLatitude != null && row.decimalLongitude != null ? {lat:Number(row.decimalLatitude), long:Number(row.decimalLongitude)} : null,
    provenance: {source_name:source.source_name, source_url:source.source_url || null, source_snapshot_date:source.snapshot_date || null, raw_record_id:sourceId || null}
  };
}
function bankCheckForTaxon(taxon, bank, source) {
  if (!source || source.snapshot_loaded !== true) return {BANK_ID:bank.BANK_ID,state:'BANCO_NO_COMPROBADO_PARA_EL_TAXON',reason:'SOURCE_SNAPSHOT_NOT_LOADED'};
  const matches = sourceMatchesTaxon(taxon, source);
  if (matches.length) return {BANK_ID:bank.BANK_ID,state:'COMPROBADO_CON_ACCESION',search_executed:true,audit_source_available:true,accepted_name_used:true,authorized_synonyms_checked:true,match_count:matches.length,source_name:source.source_name,source_url:source.source_url || null,snapshot_date:source.snapshot_date || null};
  if (source.snapshot_complete_for_holdings === true && source.current_holding_snapshot === true && source.bank_current_active_verified === true) return {BANK_ID:bank.BANK_ID,state:'COMPROBADO_SIN_ACCESION',search_executed:true,audit_source_available:true,accepted_name_used:true,authorized_synonyms_checked:true,match_count:0,source_name:source.source_name,source_url:source.source_url || null,snapshot_date:source.snapshot_date || null};
  return {BANK_ID:bank.BANK_ID,state:'BANCO_NO_COMPROBADO_PARA_EL_TAXON',reason:'SNAPSHOT_NOT_COMPLETE_AND_CURRENT_ENOUGH_FOR_NEGATIVE_ASSERTION',source_name:source.source_name,source_url:source.source_url || null,snapshot_date:source.snapshot_date || null};
}
function executeTaxon({taxon, registry, sources}) {
  ...
  const bankChecks = [], accessions = [];
  for (const bank of active) {
    const source = byBank.get(bank.BANK_ID), check = bankCheckForTaxon(taxon, bank, source);
    bankChecks.push(check);
    if (source && check.state === 'COMPROBADO_CON_ACCESION') for (const row of sourceMatchesTaxon(taxon, source)) accessions.push(normalizeAccession(row, source, taxon));
  }
...
function loadSources(manifest, baseDir) {
  return (manifest.sources || []).map(s => {
    const z={...s};
    if (z.snapshot_path) { const p=path.resolve(baseDir || '.', z.snapshot_path); z.rows=z.snapshot_format==='json'?loadJson(p):loadJsonl(p); z.snapshot_loaded=true; }
    else z.rows=[];
    return z;
  });
}
function runFullUniverse({universe, registry, sourceManifest, baseDir='.', core=null, updatedAt=null}) {
  const sources=loadSources(sourceManifest,baseDir), matrix=universe.map(taxon=>executeTaxon({taxon,registry,sources}));
  ...
  const allTaxaMatrixComplete=matrix.every(matrixCompleteForTaxon), freezeEligible=registry.active_universe_complete===true && allTaxaMatrixComplete;
  ...
  return {run_id:sourceManifest.run_id,universe_count:universe.length,active_bank_count:(registry.banks||[]).filter(b=>b.denominator_eligible===true&&b.estado==='CONFIRMADO_ACTIVO'&&b.trabaja_flora_silvestre===true).length,active_universe_complete:registry.active_universe_complete===true,all_taxa_matrix_complete:allTaxaMatrixComplete,model_scale_frozen:Boolean(snapshot),P99_A:snapshot?snapshot.P99_A:null,P99_P:snapshot?snapshot.P99_P:null,snapshot,matrix,results:finalResults};
}
if (require.main === module) {
  const args={}; for(let i=2;i<process.argv.length;i+=2) args[String(process.argv[i]||'').replace(/^--/,'')]=process.argv[i+1];
